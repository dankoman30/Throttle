...
/* Accessory output (lights). NON-safety, deliberately kept out of the
 * kill/start/throttle state machine: it simply tracks its command-flag
 * level on every valid packet, so a dropped packet only delays a change by
 * one TX period and self-heals. Cruise (CMD_FLAG_CRUISE) needs no action
 * here - the handle already froze the throttle it sent, so the value flows
 * through the normal rate-limited throttle path transparently.
 * Stored into g_aux1_state, not just written straight to a HAL call, so a
 * real board's own driver code can read the latest commanded state
 * independent of packet arrival - same externally-observed-state pattern
 * already used for g_state/g_current_servo_throttle elsewhere in this file
 * (see DEVELOPMENT/receiver/firmware/Src/bench_app.c for the precedent).
 * (A second accessory, AUX2/smoke, existed briefly and was removed
 * 2026-08-08 - not part of the handle's physical switch layout.) */
